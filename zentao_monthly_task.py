...
def main():
    ...
    for target in PROJECT_PRODUCT_TARGETS:
                ...
                for task in tasks:
                    ...
                    for act in task_detail.get("actions", []):
                        if act.get("action") == "finished":
                            ...
                            if dt and first_day_of_month <= dt <= last_day_of_month:
                                actor = act.get("actor", "")
                                if actor:
                    ...
                    if task_status != "closed":
                        ...
                        if assigned and isinstance(assigned, dict):
                            account = assigned.get("account", "")
                            if account and account not in task_members:
                                task_members[account] = {
                                    "progress": task_overall_progress,
                                    "status": "assigned",
                                }
        ...
        if finished_count:
            lines.append([{"tag": "text", "text": "🔥 本月完成任务统计（多人完成动作累计）"}])
        ...
        if pending_tasks:
            lines.append([{"tag": "text", "text": "📋 本月未关闭任务列表"}])
        ...
        if closed_tasks:
            lines.append([{"tag": "text", "text": f"✅ 本月已关闭任务（共 {len(closed_tasks)} 个）"}])
        ...
        title = (
            f"📌 本月任务统计（{format_date(first_day_of_month)} ~ {format_date(last_day_of_month)}）"
            f" [{product_name}]"
        )
